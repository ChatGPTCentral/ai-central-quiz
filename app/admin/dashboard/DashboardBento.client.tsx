...
// CTA placement thumbnails, PLACEMENT_NAME, humanizePlacement() and the
// "Which CTA gets clicked" table itself moved to components/admin/
// CtaClickedTable.client.tsx, now rendered from /admin/insights instead of
...
function VolumeMatrix({ series, gran, F, lifetimeSplits, quizRepeatTrials, preWindowAnnuals }: {
  ...
  const stations: {
    ...
    {
      // DOOR B: campaign links, the homepage slider embed, LinkedIn buttons —
    ...
    },
    {
      ...
      note: 'every trial sold in the period. ONE clock for all three rows below: the day the charge happened, cash-basis, day 1 to the last day of the period (owner, 2026-08-29) — never restated to the quiz date, so this list and /admin/revenue/trials always agree on what "this month" holds.',
      breakdown: [
        { label: 'from net-new', pick: (p: SeriesPoint) => p.netNew, tot: sumB(p => p.netNew, F.paid), metric: 'trials_net',
          note: 'took the quiz, then bought, and had never paid us before. By CHARGE date, same as the other two rows below.' },
        { label: 'from existing', pick: (p: SeriesPoint) => p.quizExistingPaid, tot: sumB(p => p.quizExistingPaid, F.quizExistingPaid), metric: 'trials_existing',
          note: 'took the quiz and then bought, but had paid us before — the quiz earned the trial, not a new customer. By CHARGE date.' },
...
export default function DashboardBento({ rows, sample, funnelEvents, series, pct, otherPaid, quizExistingPaid, lifetimeSplits, quizRepeatTrials, preWindowAnnuals }: {
  rows: BentoRow[]; sample: 'launch' | 'all'; funnelEvents: FunnelEventCounts; series: Series; pct: boolean
  ...
  const F = {
    ...
    bThenCompleted: funnelEvents.bThenCompleted, bThenClicked: funnelEvents.bThenClicked, cThenClicked: funnelEvents.cThenClicked,
  }
