...
export default function ChargeAnnual({ customerId, personKey, chargeId, trialCents }: {
  ...
  if (phase === 'error') return (
    <span style={{ display: 'inline-flex', gap: 5, alignItems: 'flex-start', whiteSpace: 'normal' }}>
      ...
      {/* Every FAILURE earns the invoice fallback — declined cards, 3DS
          requires-action banks (the hosted invoice page lets the customer
          complete the bank challenge themselves, which a merchant-initiated
          charge never can), dead Link accounts, generic failures. Only
          REFUSALS don't: those mean the row does not belong in the queue at
          all, and inviting an invoice would work around a guard. */}
